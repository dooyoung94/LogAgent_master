...
from .phase3_contract import (
    DEFAULT_PHASE3_CONFIG, NliEvidence, Phase3Error, REQUIRED_CELL_COLUMNS,
    ShortlistPolicy, TriStateThresholds, _load_json, stable_case_split,
    validate_phase3_config,
)
from .phase3_nli import (
    _compact_runtime_context, _load_cell_candidates, _load_evaluator_sets,
    _score_all_candidates, classify_tri_state,
)
from .phase3_policy import (
    _baseline_aggregate, _delta, _evaluate_policy_on_cells, apply_policy,
    evaluate_shortlist, select_calibrated_policy,
)
...
def run_phase3(
    *,
    ...
    calibration_cases, heldout_cases, split_hashes = stable_case_split(
        ...
        revision=revision,
        calibration_incidents=int(config["calibration_split"]["calibration_incidents"]),
    )
    ...
    if len(frame) != int(phase2_contract["required_cells"]):
        raise Phase3Error(f"Phase-2 cell count must be {phase2_contract['required_cells']}")
    if frame["case"].nunique() != int(phase2_contract["required_incidents"]):
        raise Phase3Error("Phase-2 incident count differs from the A3 contract")
    if set(frame["seed"].astype(int)) != {int(value) for value in phase2_contract["required_seeds"]}:
        raise Phase3Error("Phase-2 seed grid differs from the A3 contract")
    if set(frame["mask_ratio"].astype(float)) != {float(value) for value in phase2_contract["required_mask_ratios"]}:
        raise Phase3Error("Phase-2 mask ratios differ from the A3 contract")
    if int(frame["a2_proposal_count"].max()) > int(phase2_contract["a2_max_proposals"]):
        raise Phase3Error("Phase-2 proposal count exceeds the frozen A2 budget")
    truthy = {"true", "1", "yes", "y"}
    if not frame["leakage_checks_all_pass"].astype(str).str.lower().isin(truthy).all():
        raise Phase3Error("Phase-2 leakage checks did not all pass")
    if not frame["cell_gate_passed"].astype(str).str.lower().isin(truthy).all():
        raise Phase3Error("Phase-2 cell gates did not all pass")

    # Model stage first: evaluator-private targets are intentionally not loaded
    # until every A2 candidate has received its frozen NLI evidence.
    cells: list[dict[str, Any]] = []
    source_rows: dict[tuple[str, int, str], Mapping[str, Any]] = {}
    for row in frame.to_dict(orient="records"):
        candidates = _load_cell_candidates(phase2_root, row)
        key = (str(row["case"]), int(row["seed"]), str(row["mask_id"]))
        source_rows[key] = row
        cells.append({
            "case": key[0],
            "fault": str(row["fault"]),
    ...
    scoring_diagnostics = _score_all_candidates(
        cells, backend=backend, thresholds=thresholds
    )

    calibration_cells = [cell for cell in cells if cell["role"] == "calibration"]
    heldout_cells = [cell for cell in cells if cell["role"] == "heldout"]

    # Calibration labels enter first and are the only evaluator data visible
    # while selecting the policy. Held-out labels are loaded only after freeze.
    for cell in calibration_cells:
        row = source_rows[(cell["case"], cell["seed"], cell["mask_id"])]
        keys = {(item["subject"], item["predicate"], item["object"]) for item in cell["candidates"]}
        cell["targets"], cell["silver"] = _load_evaluator_sets(phase2_root, row, keys)
    policy, calibration_grid = select_calibrated_policy(
        calibration_cells,
        search=config["policy_search"],
        calibration_gate=config["calibration_gate"],
    )
    control_policy = ShortlistPolicy(policy.retention_fraction, policy.minimum_keep, 0.0)
    ...
    control_rows, control_all = _evaluate_policy_on_cells(cells, control_policy)
    proposed_cal_rows, proposed_cal = _evaluate_policy_on_cells(calibration_cells, policy)
    proposed_held_rows, proposed_held = _evaluate_policy_on_cells(heldout_cells, policy)
    control_held_rows, control_held = _evaluate_policy_on_cells(heldout_cells, control_policy)
    ...
    count_ratio = proposed_held["selected_count_mean"] / baseline_held["selected_count_mean"]
    ...
    conditions = {
        ...
        "recall_macro": proposed_held["recall_macro"] >= float(gate_cfg["recall_macro_min"]),
        "recall_pooled": proposed_held["recall_pooled"] >= float(gate_cfg["recall_pooled_min"]),
        "recall_each_cell": proposed_held["recall_min"] >= float(gate_cfg["recall_each_cell_min"]),
        "candidate_count_reduced": count_ratio <= float(gate_cfg["selected_count_ratio_max"]),
        "p_lb_improved": (
        ...
        "mrr_improved": (
            ...
            >= baseline_held["mrr_macro"] + float(gate_cfg["mrr_macro_delta_min"])
        ),
        ...
        "matched_budget_additive_gain": (
            matched_budget_additive_gain
            if bool(gate_cfg.get("matched_budget_additive_gain_required", True))
            else True
        ),
        "a2_evidence_preserved": sum(len(cell["candidates"]) for cell in cells) == int(frame["a2_proposal_count"].sum()),
    }
        ...
        for record in scored:
            detailed_scores.append({
                "incident_id": hashlib.sha256(
                    f"{revision}|task-a-phase3-output|{cell['case']}".encode("utf-8")
                ).hexdigest()[:24],
                "case": cell["case"],
                "fault": cell["fault"],
                "role": cell["role"],
                ...
                "reverse_supporting_traces": record["reverse_supporting_traces"],
                "reverse_boundary_spans": record["reverse_boundary_spans"],
                "nli_state": record["nli_state"],
                "nli_evidence_score": record["nli_evidence_score"],
                "nli_forward_entailment": record["nli_forward_entailment"],
                "nli_reverse_entailment": record["nli_reverse_entailment"],
                "nli_forward_contradiction": record["nli_forward_contradiction"],
                "nli_forward_neutral": record["nli_forward_neutral"],
                "nli_direction_margin": record["nli_direction_margin"],
                "nli_label_margin": record["nli_label_margin"],
                "a3_score": record["a3_score"],
                "selected": record["selected"],
                "is_masked_target": (record["subject"], record["predicate"], record["object"]) in cell["targets"],
                "is_silver_matched": (record["subject"], record["predicate"], record["object"]) in cell["silver"],
                "cell_selected_count": policy_by_cell[key]["selected_count"],
            })
    ...
    model_columns = [
        column for column in detailed_frame.columns
        if column not in evaluator_only_columns
    ]
    detailed_frame[model_columns].to_parquet(
        model_dir_out / "a3_candidate_evidence.parquet", index=False
    )
    detailed_frame.to_parquet(
        evaluator_dir_out / "a3_candidate_analysis.parquet", index=False
    )
    pd.DataFrame.from_records(proposed_rows).to_csv(evaluator_dir_out / "a3_cells.csv", index=False)
    pd.DataFrame.from_records(control_rows).to_csv(evaluator_dir_out / "a2_budget_control_cells.csv", index=False)
    pd.DataFrame.from_records(calibration_grid).to_csv(evaluator_dir_out / "calibration_grid.csv", index=False)
    ...
    summary = {
        ...
        "phase2_cells_sha256": hashlib.sha256(cells_path.read_bytes()).hexdigest(),
        "split": {
        ...
        "proposed_a3": {
            ...
            "heldout_delta_vs_a2": _delta(proposed_held, baseline_held),
        },
        ...
        "state_distribution": {
            state: sum(record["nli_state"] == state for record in detailed_scores)
        ...
        "claim_limit": (
            "A3 evaluates tri-state NLI-assisted shortlisting of frozen A2 CALLS "
            "proposals on six RCAEval TrainTicket incidents. It does not establish "
            "causal-edge recovery or RCA/LLM improvement."
        ),
    }
...
__all__ = [
    ...
    "ShortlistPolicy", "TriStateThresholds", "_compact_runtime_context",
    "apply_policy", "classify_tri_state", "evaluate_shortlist", "run_phase3",
    "select_calibrated_policy", "stable_case_split", "validate_phase3_config",
]
